...
class Projects extends Component {
  ...
  filterProjects(pfilter){
    ...
    if(currentFilter.includes(pfilter)){
      ...
      currentFilter=[...currentFilter.slice(0,indexOfFilter),...currentFilter.slice(indexOfFilter+1)]
    }
    else{
    ...
    */
    this.props.filterCallBack(currentFilter)
      .then((response)=>{
  ...
  generateButtons(){
    ...
    const buttons = filterButtons.map((b)=>{
      if(this.props.projectFilter.includes(b)){
        return (
          <button key={b} className="selected" onClick={()=>this.filterProjects(b,filterButtons)}> {b} <i className="fa fa-times-circle" aria-hidden="true"></i></button>
        )
      }
      else{
        return(
          <button key={b} onClick={()=>this.filterProjects(b,filterButtons)}> {b} </button>
        )
      }
    })
      ...
      if(this.state.activeProject !== project.id){
        return (
              <div key={project.id} className="project overview">
                <img className="projimg" src={project.imgLink} onClick={()=>this.activeProject(project.id)} alt={project.title}/>
                <div className="projdesc">
                  <div className="projtitle"><p>{project.title}</p> <i className="fa fa-cog" aria-hidden="true" onClick={()=>this.activeProject(project.id)}></i></div>
        ...
        const hostMessage = project.hosting ? <p className="hosting">{project.hosting}</p> : null
        return (
          <div key={project.id} className="project detail">
              <div className="detailtop">
                  <p className="detailtop title">{project.summary}</p>
                  <i className="fas fa-times" aria-hidden="true"  onClick={()=>this.activeProject(project.id)}></i>
              </div>
              {this.projectDetailBuild(project.details)}
              <hr className="divider"/>
              <div className="detailbottom">
                <a href={project.projectLink} target="_blank"><i className="fas fa-external-link-square-alt" aria-hidden="true"></i> </a>
                {hostMessage}
                <a href={project.githubLink} target="_blank"><i className="fab fa-github-square" aria-hidden="true"></i> </a>
              </div>
          </div>
        )
      }
