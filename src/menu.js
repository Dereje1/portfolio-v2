...
class Menu extends Component {
  ...
  scrollUp(){//must refactor
      const allSections = ['containAbout','containProjects','containSkills','containEducation','containContact']
      const expandedSections = allSections.filter((s)=>{//filter out unexpanded sections
          return document.getElementById(s)
      })
      if(!expandedSections.length){return}//all sections collapsed
      let filteredSections = expandedSections.filter((section)=>{//filter out sections outside of viewport
        let boundingBox = document.getElementById(section).getBoundingClientRect()
      ...
      let ans = (filteredSections[filteredSections.length-1].split("contain"))[1].toLowerCase()

      let beforeScrollBounding = document.getElementById(filteredSections[filteredSections.length-1]).getBoundingClientRect()

      let contentBox = document.getElementById("content").getBoundingClientRect()

      if(beforeScrollBounding.top>200 && beforeScrollBounding.top<202){//already scrolled up(100 = section heading height, 75 main header height)
  ...
  hideSection(toHide){
    ...
    if(currentlyHidden.includes(toHide)){//unhide
      const indexOfHidden = currentlyHidden.findIndex((s) => toHide===s)
      currentlyHidden=[...currentlyHidden.slice(0,indexOfHidden),...currentlyHidden.slice(indexOfHidden+1)]
  ...
  controlScroll(){
    if(this.state.hiddenSection.length === 5 ){return null}
    if(this.state.lastScrollTime > 2){return null}
    const scrollerButton = <div className="scroller" onClick={this.scrollUp}><i className="far fa-arrow-alt-circle-up"></i></div>
