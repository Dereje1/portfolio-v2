...
class Content extends Component {
  ...
  componentDidUpdate(prevProps, prevState){
    if(prevProps.scrollpoint!==this.props.scrollpoint){
      if(!this.props.scrollpoint){return}
      let off = (this.props.addMargin==="top left" || this.props.addMargin==="top") ? -100 : 0
  ...
  sectionHeader(section){
    if (this.props.hidden.includes(section)){
      return (
        <div id={section} className="sectiondiv" onClick={()=>this.props.sectionHide(section)}>{section}
  ...
  render() {
    return (
      <div id="content" className={this.props.addMargin} onClick={this.props.contract}>
